    ...
    <q-header>
      <q-toolbar>
        ...
        <q-btn class="gt-xs" no-caps color="grey-1" text-color="grey-9" icon="eva-plus-circle"
          :to="{ name: 'PostCreate' }" label="Tambah Post" />
      </q-toolbar>
    </q-header>
        ...
        <q-list separator>
          <q-item v-for="post in posts.data" :key="post.id">
            ...
            <q-item-section avatar top class="q-pr-sm">
              <img v-if="post.image" :src="post.image_url" class="img-thumb" />
            </q-item-section>
            ...
            <q-item-section top>
              ...
              <div class="q-mt-xs">
                <q-chip dense v-if="post.is_listing" label="Listing" size="sm" color="accent" text-color="white"
                  icon="eva-checkmark-circle-2"></q-chip>
                <q-chip dense v-if="post.is_promote" label="Promote" size="sm" color="teal" text-color="white"
                  icon="eva-checkmark-circle-2"></q-chip>
              </div>
            ...
            <q-item-section side top>
              <div class="text-grey-8 column q-gutter-y-sm">
                ...
                <q-btn :to="{ name: 'PostEdit', params: { id: post.id } }" size="sm" round glossy color="info"
                  icon="eva-edit-2" />
              </div>
            </q-item-section>
          </q-item>
