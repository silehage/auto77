<template>
  ...
  >
    ...
    <q-footer v-if="carts.items.length" class="bg-white q-pa-md">
      ...
      <q-btn unelevated @click="checkout" color="primary" class="full-width q-mb-sm" no-caps>
        <svg
      ...
      <q-btn q-btn outline v-if="isCanCheckoutWhatsapp" unelevated @click="checkoutWhatsapp" color="green-6" class="full-width" no-caps>
...
export default {
  ...
  computed: {
    ...
    isCanCheckoutDirectWithShipping() {
       if(this.carts.items.length && this.shop.phone) {
