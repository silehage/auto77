   ...
   <q-page class="q-pb-xl" :class="{'flex flex-center' : !galleries.available}">
    <q-header>
      <q-toolbar>
        ...
        <q-btn class="gt-xs" outline icon="eva-plus-circle" label="Upload Gallery" no-caps @click="handleBtnUpload"/>
      </q-toolbar>
    </q-header>
    <div class="q-pa-sm text-xs text-grey-8 bg-yellow-2 z-50" :class="galleries.data.length? 'relative' : 'absolute-top'">
      <div>Note</div>
      ...
      <div class="q-py-md">
        ...
        <q-item v-for="gallery in galleries.data" :key="gallery.id">
          <q-item-section top avatar>
            <img :src="gallery.src" class="shadow-4 q-pa-xs bg-white" style="width:100px;height:70px;object-fit:cover;"/>
          </q-item-section>
          <q-item-section top>
